...
from lerobot.policies.factory import make_pi05_raw32_features
...
def test_raw32_features_exclude_joint_state_and_actions() -> None:
    ...
    inputs, outputs = make_pi05_raw32_features(metadata, RENAME_MAP)

    assert inputs[OBS_STATE].shape == (32,)
    assert outputs == {ACTION: outputs[ACTION]}
    assert outputs[ACTION].shape == (32,)
    assert "state" not in inputs
    assert "actions" not in outputs
    assert all(feature.type is FeatureType.VISUAL for key, feature in inputs.items() if key != OBS_STATE)


def test_raw32_stats_are_canonical_and_normalization_round_trips() -> None:
    canonical_stats = make_pi05_raw32_stats(
        {
            "ee_state": _stats(44),
            "ee_actions": _stats(34),
            "state": _stats(44),
            "actions": _stats(34),
        }
    )
    assert set(canonical_stats) == {OBS_STATE, ACTION}
    assert canonical_stats[OBS_STATE]["mean"].shape == (32,)
    assert canonical_stats[ACTION]["mean"].shape == (32,)

    cfg = PI05Config(device="cpu", dtype="float32")
    features = {
        OBS_STATE: SimpleNamespace(type=FeatureType.STATE, shape=(32,)),
        ACTION: SimpleNamespace(type=FeatureType.ACTION, shape=(32,)),
    }
    ...
    transition = pi05_raw32_batch_to_transition(
        {
            "ee_state": torch.arange(44, dtype=torch.float32).unsqueeze(0) + 1,
            "ee_actions": torch.arange(34, dtype=torch.float32).reshape(1, 1, 34) + 1,
            "top_head": torch.rand(1, 3, 8, 8),
            "task": ["pick"],
        }
    )
...
def test_raw32_processor_checkpoint_round_trip(mock_tokenizer, tmp_path) -> None:
    ...
    stats = make_pi05_raw32_stats(
        {"ee_state": _stats(44), "ee_actions": _stats(34)}
    )
